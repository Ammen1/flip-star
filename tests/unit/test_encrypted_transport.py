...
These tests build disposable views with APIRequestFactory rather than
touching a real URL, and need no database, so they exercise the
parser/renderer/decorator contract in isolation.

(This docstring used to say "no real endpoint uses these yet". That is no
longer true: api/views/wallet.py applies @encrypted_endpoint to
wallet_summary and wallet_transactions. The stale claim mattered -- an e2e
test was still asserting a plaintext 200 from /api/v1/wallet/ and failing
with 400 against the encryption the endpoint had since gained. See
tests/e2e/test_health.py::test_authenticated_wallet_summary_round_trip,
which now performs a real encrypted round trip against that URL.)
"""
...
def test_parse_missing_envelope_field_raises(_server_keys, client_keys, missing_field):
    ...
    with pytest.raises(DecryptionError):
        EncryptedJSONParser().parse(
            ...
            parser_context=_parser_context(request),
        )
...
def test_parse_tampered_ciphertext_raises(_server_keys, client_keys):
    ...
    envelope['encrypted'] = envelope['encrypted'][:-4] + (
        'AAAA' if envelope['encrypted'][-4:] != 'AAAA' else 'BBBB'
    )
    request = factory.post('/x/', HTTP_X_CLIENT_PUBLIC_KEY=client_public_key)
