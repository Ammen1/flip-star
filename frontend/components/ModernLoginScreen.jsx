    ...
    <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 14, fontSize: 11 }}>
      <thead>
        <tr style={{ background: "#1A1A1A" }}>
          {headers.map((h, i) => (
            <th key={i} style={{ padding: "7px 8px", color: "#C8B56A", fontWeight: 700, textAlign: "left", border: "1px solid #333", lineHeight: 1.4 }}>{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
...
function TermsModal({ onClose }) {
  const sec  = { fontSize: 14, fontWeight: 800, color: "#C8B56A", marginTop: 20, marginBottom: 8 };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.97)", zIndex: 9999, overflowY: "auto" }}>
      <div style={{ background: "#111", minHeight: "100vh", width: "100%", maxWidth: 640, margin: "0 auto", padding: "48px 24px 60px" }}>
        {/* Header */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div style={{ fontSize: 22, fontWeight: 900, color: "#C8B56A" }}>Terms &amp; Conditions</div>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "#C8B56A" }}><X size={24} /></button>
        </div>
    ...
    <div style={{ minHeight: "100vh", background: T.bg || "#0D0D0D", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px 16px" }}>
      ...
      <div style={{ width: "100%", maxWidth: 420 }}>
        ...
        <div style={{ 
          ...
          <img src="/static/images/70x20.png" alt="Logo" style={{ width: 140, height: 40, objectFit: "contain" }} />
        </div>
