...
from api.models import Comment, CommentLike, CommentReply, Mention, Reel, SavedPost
from api.serializers.extended import (
        ...
        for username in usernames:
            try:
                ...
                if mentioned_user.id in blocked_ids:
                    continue
                if (
                    not hasattr(mentioned_user, 'profile')
                    or not mentioned_user.profile.allow_mentions
                ):
                    continue
...
class CommentViewSet(EncryptedPayloadMixin, viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if comment.is_deleted:
            return Response(
                {'error': 'Cannot edit a deleted comment'}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            if 'does not exist' in str(e) or 'column' in str(e).lower():
                ...
                reply = CommentReply.objects.create(
                    user=request.user, comment=comment, text=text, edited_at=None, is_deleted=False
                )
            else:
                raise
...
class CommentReplyViewSet(EncryptedPayloadMixin, viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if reply.user != request.user:
            return Response(
                {'error': 'You can only edit your own replies'}, status=status.HTTP_403_FORBIDDEN
            )
        # No time-based edit window — owners may edit their replies at any time.
        if reply.is_deleted:
            return Response(
                {'error': 'Cannot edit a deleted reply'}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if reply.user != request.user:
            return Response(
                {'error': 'You can only delete your own replies'}, status=status.HTTP_403_FORBIDDEN
            )
    ...
    def like(self, request, pk=None):
        try:
            reply = self.get_object()
            like, created = CommentLike.objects.get_or_create(user=request.user, reply=reply)
            if not created:
                like.delete()
...
class SavedPostViewSet(EncryptedPayloadMixin, viewsets.ModelViewSet):
    ...
    def toggle(self, request):
        ...
        saved, created = SavedPost.objects.get_or_create(user=request.user, reel=reel)
