...
def download_data(request):
    ...
    user = request.user
    from api.models import UserProfile, Reel, Comment, Follow
...
def register_with_phone(request):
    ...
    from api.models.subscription import SubscriptionPlan as UserSubscription, SubscriptionPayment, SubscriptionHistory
    from django.db import transaction
    ...
    if not skip_otp:
        ...
        print(f"[REGISTRATION DEBUG] Verification marker found")
    ...
    try:
        ...
        if not skip_otp:
            ...
            print(f"[REGISTRATION DEBUG] Verification marker consumed")
...
def resend_subscription_otp(request):
    ...
    safe_response = {'message': 'If an active subscription exists, a new OTP has been sent via SMS.', 'phone': mask_phone_number(phone)}
    ...
    if not OnevasWebhookView().send_sms(sms_phone, message, tier.duration_type):
        return Response({'error': 'Failed to send OTP SMS. Please try again.'}, status=status.HTTP_502_BAD_GATEWAY)
...
def dev_create_subscription(request):
    ...
    from api.models.subscription import SubscriptionPlan as UserSubscription, SubscriptionTier
    phone_raw = request.data.get('phone', '').strip()
    phone = _normalize_ethiopian_phone(phone_raw)
    if not phone:
...
def forgot_password_phone_request(request):
    ...
    try:
        ...
        if active_subscription and active_subscription.tier:
            ...
            print(f"[PWD-RESET-PHONE] No active subscription, using configured Onevas keys")
    except Exception as e:
        print(f"[PWD-RESET-PHONE] Error getting subscription: {e}")
        application_key = _settings.ONEVAS_APPLICATION_KEY
        product_number = _settings.ONEVAS_PRODUCT_NUMBER
        print(f"[PWD-RESET-PHONE] Using configured Onevas keys")
    ...
    success, message = OTPService.send_otp(phone, application_key, product_number, action='password_reset')
...
class FollowViewSet(EncryptedPayloadMixin, viewsets.ModelViewSet):
    ...
    def suggestions(self, request):
        ...
        if request.user.is_authenticated:
            ...
            from api.models import Reel, Vote, Comment
