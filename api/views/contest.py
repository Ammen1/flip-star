...
def upgrade_subscription(request):
    ...
    if tier == 'free':
        return Response(
            {'error': 'Cannot upgrade to free tier'}, status=status.HTTP_400_BAD_REQUEST
        )
...
def send_gift(request):
    ...
    if not recipient_username:
        return Response(
            {'error': 'recipient_username is required'}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    Notification.objects.create(
        recipient=recipient, sender=sender, notification_type='gift', message=notification_message
    )
...
def purchase_extra_entry(request):
    ...
    if ExtraEntryPurchase.objects.filter(user=user, date=today).exists():
        return Response(
            {'error': 'Already purchased extra entry today'}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    ExtraEntryPurchase.objects.create(
        user=user, date=today, extra_entries_purchased=1, coins_spent=cost
    )
...
def get_post_score(request, reel_id):
    ...
    try:
        ...
        score, _ = ContestPostScore.objects.get_or_create(reel=reel, user=reel.user, defaults={})
    except Reel.DoesNotExist:
        return Response({'error': 'Post not found'}, status=status.HTTP_404_NOT_FOUND)
...
def calculate_leaderboard(period, date):
    ...
    if period == 'daily':
        ...
        # Get top 10 + 5 random
        top_scores = ContestPostScore.objects.filter(created_at__date=date).order_by(
            '-total_score'
        )[:10]

        # Get 5 random from remaining
        remaining = (
            ContestPostScore.objects.filter(created_at__date=date)
            .exclude(id__in=[s.id for s in top_scores])
            .order_by('?')[:5]
        )
        ...
        # Aggregate by user
        user_scores = (
            ContestPostScore.objects.filter(created_at__date__gte=start, created_at__date__lt=end)
            .values('user')
            .annotate(total=Sum('total_score'), post_count=Count('id'))
            .order_by('-total')[:5]
        )
    ...
    try:
        ...
        response_data = {
            ...
            'budget': {
                ...
                - (
                    float(contest.spent_daily)
                    + float(contest.spent_weekly)
                    + float(contest.spent_monthly)
                    + float(contest.spent_grand)
                ),
            },
            'stats': {
        ...
        print('[CONTEST DASHBOARD] Success, returning data')
...
def verify_phone(request):
    ...
    if not verification_code:
        ...
        # secrets, not random: this is an authentication credential, and
        # random's Mersenne Twister is reconstructable from a handful of
        # observed outputs, which would make other users' codes predictable.
        import secrets
...
def vote_grand_finale(request):
    ...
    if contest.get_days_remaining() != 0:
        return Response(
            {'error': 'Grand finale voting is not open'}, status=status.HTTP_400_BAD_REQUEST
        )
...
def check_upload_eligibility(request):
    ...
    try:
        ...
        if not subscription.can_post_today():
            ...
            today = timezone.now().date()
            extra_purchased = ExtraEntryPurchase.objects.filter(user=user, date=today).exists()
