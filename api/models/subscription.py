...
class AdminRole(models.Model):
    ...
    def has_permission(self, permission):
        ...
        `permissions['custom']` is an explicit per-admin override and
        bypasses the permission_level restriction below -- that's the point
        of granting it individually rather than through the role.
        """
        if self.role == 'super_admin':
            return True

        if permission in self.permissions.get('custom', []):
            return True
