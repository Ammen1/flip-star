...
class Category(models.Model):
    ...
    icon = models.CharField(
        max_length=50, blank=True, help_text='Icon name (e.g., dance, comedy, etc.)'
    )
...
class UserProfile(models.Model):
    ...
    points_withdrawn_total = models.IntegerField(
        default=0, help_text='Total points withdrawn lifetime'
    )
    ...
    allow_mentions = models.BooleanField(
        default=True, help_text='Allow other users to mention you in comments'
    )
    ...
    last_gift_reset = models.DateField(
        null=True, blank=True, help_text='Last daily gift counter reset'
    )
    ...
    has_used_free_trial = models.BooleanField(
        default=False, help_text='User has used their 1-day free trial'
    )

    # Trial tracking
    trial_start_date = models.DateTimeField(
        null=True, blank=True, help_text='When 3-day trial started'
    )
    ...
    trial_interaction_count = models.IntegerField(
        default=0, help_text='How many interactions attempted'
    )

    # OTP tracking
    # noqa DJ001: null=True on a CharField is the wrong shape, but changing it
    # now needs a data migration turning existing NULLs into '' on a live
    # auth field. Left as-is deliberately rather than fixed in passing.
    otp_code = models.CharField(max_length=6, blank=True, null=True)  # noqa: DJ001
    ...
    is_shadowbanned = models.BooleanField(
        default=False,
        help_text='User is shadow banned - content hidden from others but visible to self',
    )
    ban_expires_at = models.DateTimeField(
        null=True, blank=True, help_text='When temporary ban expires (null if not temp banned)'
    )
...
class Reel(models.Model):
    ...
    campaign = models.ForeignKey(
        'Campaign', on_delete=models.SET_NULL, null=True, blank=True, related_name='campaign_posts'
    )
    ...
    total_boost_impressions = models.IntegerField(
        default=0, help_text='Total impressions from all boost campaigns'
    )
    total_boost_engagements = models.IntegerField(
        default=0, help_text='Total engagements from all boost campaigns'
    )

    # Moderation
    is_hidden = models.BooleanField(
        default=False, help_text='Content is hidden/removed by moderation (soft-delete)'
    )
...
class CommentLike(models.Model):
    ...
    comment = models.ForeignKey(
        Comment, on_delete=models.CASCADE, related_name='comment_likes', null=True, blank=True
    )
    reply = models.ForeignKey(
        'CommentReply', on_delete=models.CASCADE, related_name='reply_likes', null=True, blank=True
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class CommentReply(models.Model):
    ...
    def __str__(self):
        parent = (
            f'reply {self.parent_reply.id}' if self.parent_reply else f'comment {self.comment.id}'
        )
...
class Mention(models.Model):
    ...
    comment = models.ForeignKey(
        Comment, on_delete=models.CASCADE, related_name='mentions', null=True, blank=True
    )
    reply = models.ForeignKey(
        CommentReply, on_delete=models.CASCADE, related_name='mentions', null=True, blank=True
    )
...
class Winner(models.Model):
    ...
    reel = models.ForeignKey(
        Reel, on_delete=models.CASCADE, related_name='wins', null=True, blank=True
    )
...
class Report(models.Model):
    ...
    reported_reel = models.ForeignKey(
        Reel, on_delete=models.CASCADE, related_name='reports', null=True, blank=True
    )
...
class ModerationAction(models.Model):
    ...
    moderator = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name='moderation_actions_taken'
    )
    ...
    undone_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='moderation_actions_undone',
    )
