...
class BoostConfig(models.Model):
    ...
    base_impression_rate = models.IntegerField(
        default=10, help_text='Base impressions per coin (1 coin = X impressions)'
    )

    # Platform fee
    platform_fee_percent = models.IntegerField(
        default=10, help_text='Platform fee percentage on top of boost cost'
    )
    ...
    max_duration_hours = models.IntegerField(
        default=168, help_text='Maximum boost duration in hours (7 days)'
    )
    ...
    # Frequency capping
    frequency_cap_hours = models.IntegerField(
        default=12, help_text='Hours before same user can see same boost again'
    )

    # User limits
    max_daily_boosts_per_user = models.IntegerField(
        default=5, help_text='Max boosts per day per user'
    )
    max_active_boosts_per_post = models.IntegerField(
        default=3, help_text='Max concurrent boosts per post'
    )

    # Premium targeting surcharge
    premium_targeting_surcharge = models.IntegerField(
        default=20, help_text='Additional % cost for premium targeting (age/gender/location)'
    )

    # Refund policy
    refund_threshold_percent = models.IntegerField(
        default=50, help_text='If less than X% of expected impressions served, refund X%'
    )

    cancellation_fee_percent = models.IntegerField(
        default=10, help_text='Cancellation fee percentage of remaining budget'
    )
    ...
    def calculate_cost(self, duration_hours, has_premium_targeting=False):
        ...
        if has_premium_targeting:
            discounted_cost = discounted_cost * (
                Decimal('1') + Decimal(self.premium_targeting_surcharge) / Decimal('100')
            )

        # Add platform fee
        final_cost = discounted_cost * (
            Decimal('1') + Decimal(self.platform_fee_percent) / Decimal('100')
        )
...
class BoostCampaign(models.Model):
    ...
    engagement_count = models.IntegerField(
        default=0, help_text='Total engagements (likes + comments + shares)'
    )
    ...
    hourly_budget = models.DecimalField(
        max_digits=10, decimal_places=2, help_text='Coins per hour budget'
    )
    ...
    def is_active(self):
        ...
        return (
            self.status == 'active' and timezone.now() < self.end_time and self.coins_remaining > 0
        )
...
class BoostImpression(models.Model):
    ...
    campaign = models.ForeignKey(
        BoostCampaign, on_delete=models.CASCADE, related_name='impressions'
    )
...
class BoostEngagement(models.Model):
    ...
    campaign = models.ForeignKey(
        BoostCampaign, on_delete=models.CASCADE, related_name='engagements'
    )
...
class BoostStats(models.Model):
    ...
    campaign = models.ForeignKey(
        BoostCampaign, on_delete=models.CASCADE, related_name='daily_stats'
    )
