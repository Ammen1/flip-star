...
class Gift(models.Model):
    ...
    image = models.ImageField(
        ...
        null=True,
        blank=True,
        help_text='Optional uploaded icon/image (overrides icon_name if set)',
    )
    animated_image = models.ImageField(
        upload_to='gifts/animated/', null=True, blank=True, help_text='Animated version of the gift'
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text='Whether this gift is available for sending'
    )
    ...
    animation_type = models.CharField(
        max_length=50, blank=True, help_text='Animation type (e.g., particle, bounce, pulse)'
    )
...
class GiftTransaction(models.Model):
    ...
    reel = models.ForeignKey(
        ...
        help_text='The reel this gift was sent on',
    )
...
class UserGiftStats(models.Model):
    ...
    unique_recipients = models.IntegerField(
        default=0, help_text='Number of different users they gifted'
    )

    # Receiving stats
    total_gifts_received = models.IntegerField(default=0)
    total_coins_received = models.IntegerField(default=0)
    unique_senders = models.IntegerField(
        default=0, help_text='Number of different users who gifted them'
    )

    # Favorite gift
    favorite_gift_sent = models.ForeignKey(
        Gift, on_delete=models.SET_NULL, null=True, blank=True, related_name='favorite_senders'
    )
    favorite_gift_received = models.ForeignKey(
        Gift, on_delete=models.SET_NULL, null=True, blank=True, related_name='favorite_receivers'
    )
...
class WinnerGiftTransaction(models.Model):
    ...
    gift_package = models.ForeignKey(
        WinnerGiftPackage, on_delete=models.PROTECT, null=True, blank=True
    )
