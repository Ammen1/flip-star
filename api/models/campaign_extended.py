...
class WinnerFrequencyRecord(models.Model):
    ...
    def check_frequency_eligibility(cls, user, winner_type, campaign=None):
        ...
        if winner_type == 'daily':
            ...
            recent_wins = cls.objects.filter(
                user=user, winner_type='daily', won_at__gte=month_start, won_at__lt=month_end,
            ).count()
            ...
            recent_wins = cls.objects.filter(
                user=user, winner_type='weekly', won_at__gte=month_start, won_at__lt=month_end,
            ).count()
            ...
            if campaign:
                recent_wins = cls.objects.filter(user=user, winner_type='monthly', campaign=campaign).count()
            else:
            ...
            if campaign:
                recent_wins = cls.objects.filter(user=user, winner_type='grand', campaign=campaign).count()
            else:
    ...
    def record_win(cls, user, winner_type, campaign=None, selection=None, period_start=None, period_end=None):
        ...
        return cls.objects.get_or_create(
            user=user, winner_type=winner_type, period_start=period_start,
