...
class UserCoinBalance(models.Model):
    ...
    - telebirr_purchased_balance: bought via Telebirr. Full use including gifting.
    - airtime_purchased_balance: bought via Onevas airtime. Cannot be gifted.

    `balance` = earned + telebirr_purchased + airtime_purchased (kept for backward compat).
    `purchased_balance` = telebirr_purchased + airtime_purchased (kept for backward compat --
    older code and API responses that only know about a single "purchased" bucket still work).
    """
    ...
    telebirr_purchased_balance = models.PositiveIntegerField(default=0, help_text='Bought via Telebirr. Full use including gifting.')
    airtime_purchased_balance = models.PositiveIntegerField(default=0, help_text='Bought via Onevas airtime. Cannot be gifted.')

    # Legacy combined purchased balance (sum of both purchase methods, kept in sync by _sync_balance).
    purchased_balance = models.PositiveIntegerField(default=0, help_text='Total purchased coins (Telebirr + airtime).')

    total_earned = models.PositiveIntegerField(default=0)
    total_spent = models.PositiveIntegerField(default=0)
    total_purchased = models.PositiveIntegerField(default=0, help_text='Lifetime coins bought')
    total_telebirr_purchased = models.PositiveIntegerField(default=0, help_text='Lifetime coins bought via Telebirr')
    total_airtime_purchased = models.PositiveIntegerField(default=0, help_text='Lifetime coins bought via Onevas airtime')
    ...
    def _sync_balance(self):
        """Keep legacy `balance` and `purchased_balance` fields in sync with the three buckets."""
        self.purchased_balance = (self.telebirr_purchased_balance or 0) + (self.airtime_purchased_balance or 0)
        self.balance = (self.earned_balance or 0) + self.purchased_balance
    ...
    def add_purchased(self, amount, transaction_type='purchase', payment_method='telebirr', **kwargs):
        """
        ...
        with transaction.atomic():
            ...
            if payment_method == 'airtime':
                ...
                locked.telebirr_purchased_balance = (locked.telebirr_purchased_balance or 0) + amount
            ...
            locked.save(update_fields=[
                'telebirr_purchased_balance', 'airtime_purchased_balance',
                'purchased_balance', 'balance', 'total_purchased',
                'total_telebirr_purchased', 'total_airtime_purchased', 'updated_at',
            ])
            coin_tx = CoinTransaction.objects.create(
                user=locked.user, transaction_type=transaction_type, coins=amount,
    ...
    def spend_coins(self, amount, transaction_type, restrict_earned=False, **kwargs):
        ...
        - If restrict_earned=True (e.g., gifting): can only use telebirr_purchased_balance
          (airtime-purchased coins cannot be gifted).
        - Otherwise: prefer earned_balance first, then airtime_purchased, then telebirr_purchased.
        ...
        with transaction.atomic():
            ...
            if restrict_earned:
                ...
                if (locked.telebirr_purchased_balance or 0) < amount:
                    raise ValueError(
                        f'Insufficient giftable coins. Have {locked.telebirr_purchased_balance} Telebirr coins, need {amount}. '
                        'Airtime-purchased coins cannot be used for gifting. Top up via Telebirr.'
                    )
                locked.telebirr_purchased_balance -= amount
            else:
                # In-app actions: prefer earned, then airtime, then telebirr
                total = (locked.earned_balance or 0) + (locked.airtime_purchased_balance or 0) + (locked.telebirr_purchased_balance or 0)
                if total < amount:
                    raise ValueError(f'Insufficient balance. Have {total}, need {amount}')

                from_earned = min(locked.earned_balance or 0, amount)
                remaining = amount - from_earned
            ...
            locked._sync_balance()
            locked.save(update_fields=[
                'earned_balance', 'telebirr_purchased_balance', 'airtime_purchased_balance',
                'purchased_balance', 'balance', 'total_spent', 'updated_at',
            ])
            coin_tx = CoinTransaction.objects.create(
                user=locked.user, transaction_type=transaction_type, coins=-amount, **kwargs
            )
    ...
    def _copy_from(self, other):
        ...
        for field in (
            'earned_balance', 'telebirr_purchased_balance', 'airtime_purchased_balance', 'purchased_balance', 'balance',
            'total_earned', 'total_purchased', 'total_telebirr_purchased', 'total_airtime_purchased',
    ...
    def deduct_from_bucket(self, bucket, amount, transaction_type='admin_adjustment', **kwargs):
        ...
        with transaction.atomic():
            ...
            locked.total_spent = (locked.total_spent or 0) + amount
            locked._sync_balance()
            locked.save(update_fields=[field, 'purchased_balance', 'balance', 'total_spent', 'updated_at'])
            coin_tx = CoinTransaction.objects.create(
                user=locked.user, transaction_type=transaction_type, coins=-amount, **kwargs
            )
    ...
    def withdraw_to_birr(self, amount):
        ...
        with transaction.atomic():
            ...
            if (locked.earned_balance or 0) < amount:
                raise ValueError(f'Insufficient earned coins. Have {locked.earned_balance}, need {amount}')
