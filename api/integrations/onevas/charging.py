...
class OnevasChargingService:
    ...
    #: Resolved from settings (environment -> Vault -> .env) rather than
    #: hardcoded, so the endpoint can be pointed at a sandbox without a code
    #: change. Kept as an instance attribute so existing call sites still work.
    def __init__(self):
        self.ONEVAS_CHARGING_URL = settings.ONEVAS_CHARGING_URL
