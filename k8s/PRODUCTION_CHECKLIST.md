...
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| Staging domain             | `k8s/overlays/staging/patches/configmap-patch.yaml` → `ALLOWED_HOSTS`, `CORS_ALLOWED_ORIGINS`, `CSRF_TRUSTED_ORIGINS`, `BACKEND_URL`                                           | `[KNOWN]` — `api.uat.flipstar.et`                   | The only live domain this project has. Also in `docker-compose.yml`/`nginx.conf`.                                              |
| Staging ingress host       | `k8s/overlays/staging/ingress.yaml`                                                                                                                                            | `[KNOWN]` — `api.uat.flipstar.et`                   | Matches the ConfigMap.                                                                                                         |
...
(OneVAS has been removed; no `ONEVAS_*` key is read. SMS needs the
`TIMWE_SMPP_*` keys — see docs/sms-smpp.md.)
